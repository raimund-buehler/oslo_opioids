...
# ============================================================
# PLOT: Model 5 simple effects: Drug differences within genotype at ASQ mean
# ============================================================
...
for (v in fac_vars2) {
  if (v %in% names(mf2) && !is.factor(mf2[[v]])) {
    mf2[[v]] <- factor(mf2[[v]])
  }
}

if ("Drug" %in% names(mf2)) {
  mf2$Drug <- factor(mf2$Drug, levels = drug_levels)
}
if ("Genetics" %in% names(mf2)) {
  mf2$Genetics <- factor(mf2$Genetics, levels = c("A","G"))
}

nuis2 <- intersect(
  c("Genetics","FaceGender","Gaze2","AttrLevel","Imagelist","Session"),
  names(mf2)
)
...
# Same plot but percentiles 25 vs 75 (instead of mean split)
...
plot_dat <- bind_rows(plot_low, plot_high) %>%
  mutate(
    ASQ_level = factor(
      ASQ_level,
      levels = c("Low ASQ (25th pct)", "High ASQ (75th pct)")
    ),
    Drug = factor(as.character(Drug), levels = drug_levels)
  ) %>%
  arrange(ASQ_level, Drug)
...
# -----------------------
# Annotation for significant post-hoc at HIGH ASQ
# -----------------------
y_high_panel <- plot_dat %>%
  filter(ASQ_level == "High ASQ (75th pct)") %>%
  summarise(y = max(upper, na.rm = TRUE)) %>%
  pull(y)

# Start y-axis at 0 so bars do not visually overlap x labels
y_base <- 0
y_max_data <- max(plot_dat$upper, na.rm = TRUE)
y_range <- y_max_data - y_base

bracket_y <- y_high_panel + 0.08 * y_range
label_y   <- bracket_y   + 0.05 * y_range
tip_y     <- bracket_y   - 0.015 * y_range
y_top     <- label_y     + 0.06 * y_range

annot_df <- tibble(
  ASQ_level = factor("High ASQ (75th pct)", levels = levels(plot_dat$ASQ_level)),
  x1 = "morphine",
  x2 = "naltrexone",
  x_mid = 2,   # midpoint between morphine (1) and naltrexone (3) with placebo in the middle
  y = bracket_y,
  y_tip = tip_y,
  y_label = label_y,
  label = "morphine vs naltrexone: OR = 2.006**"
)
...
p_h3 <- ggplot(plot_dat, aes(x = Drug, y = FixTimePerc, fill = Drug)) +
  ...
  labs(
    x = NULL,
    y = "Fixation Time (%)",
    title = "Drug effects at low vs high ASQ (Percentiles)"
  ) +
  theme_minimal(base_size = 12) +
  theme(
    legend.position = "none",
    plot.margin = margin(10, 30, 15, 20)
  ) +
  geom_segment(
    data = annot_df,
...
ggsave(file.path(out_dir, "model2_drug_by_asq_percentiles_barplot.png"), p_h3, width = 8, height = 5, dpi = 300)
